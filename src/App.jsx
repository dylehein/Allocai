                  ...
                  <div className="max-w-md mx-auto">
                    <div className="relative" style={{ width: '100%', height: '300px' }}>
                      <svg viewBox="0 0 200 200" className="w-full h-full">
                        ...
                        {(() => {
                          const totalIncome = result.income;
                          const taxes = result.total;
                          const takeHome = result.take;
                          
                          const taxPercent = (taxes / totalIncome) * 100;
                          const takeHomePercent = (takeHome / totalIncome) * 100;
                          ...
                          // Tax slice (red) - starts at top
                          const taxEndAngle = taxAngle * (Math.PI / 180);
                          const taxX = centerX + radius * Math.sin(taxEndAngle);
                          const taxY = centerY - radius * Math.cos(taxEndAngle);
                          const taxLargeArc = taxAngle > 180 ? 1 : 0;
                          
                          // Take-home slice (green) - continues from tax
                          const takeHomeStartAngle = taxAngle * (Math.PI / 180);
                          const takeHomeEndAngle = (taxAngle + takeHomeAngle) * (Math.PI / 180);
                          const takeHomeX = centerX + radius * Math.sin(takeHomeEndAngle);
                          const takeHomeY = centerY - radius * Math.cos(takeHomeEndAngle);
                          const takeHomeLargeArc = takeHomeAngle > 180 ? 1 : 0;
                          
                          return (
                            <g>
                              {/* Tax slice (red) */}
                              <path
                                d={`M ${centerX} ${centerY} L ${centerX} ${centerY - radius} A ${radius} ${radius} 0 ${taxLargeArc} 1 ${taxX} ${taxY} Z`}
                                fill="#ef4444"
                              ...
                              <path
                                d={`M ${centerX} ${centerY} L ${taxX} ${taxY} A ${radius} ${radius} 0 ${takeHomeLargeArc} 1 ${takeHomeX} ${takeHomeY} Z`}
                                fill="#22c55e"
                                className="transition-all duration-300 hover:opacity-80"
                              />
                              ...
                              {/* Center text */}
                              <text x={centerX} y={centerY - 5} textAnchor="middle" className="text-xs fill-slate-600 font-medium">
                                Your
                              </text>
                              <text x={centerX} y={centerY + 10} textAnchor="middle" className="text-2xl fill-slate-900 font-bold">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-6">
                      <div className="flex items-center gap-3 p-4 bg-red-50 rounded-2xl">
                        ...
                        <div>
                          <p className="text-sm text-slate-600">Taxes</p>
                          <p className="text-xl font-bold text-red-600">${result.total.toFixed(2)}</p>
                          <p className="text-xs text-slate-500">{result.rate.toFixed(1)}%</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 p-4 bg-green-50 rounded-2xl">
                        ...
                        <div>
                          <p className="text-sm text-slate-600">Take-Home</p>
                          <p className="text-xl font-bold text-green-600">${result.take.toFixed(2)}</p>
                          <p className="text-xs text-slate-500">{((result.take / result.income) * 100).toFixed(1)}%</p>
                        </div>
                      </div>
                    </div>
                  </div>
